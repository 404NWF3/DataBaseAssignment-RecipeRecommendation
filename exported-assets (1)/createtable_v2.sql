-- =====================================================================
-- AllRecipes 食谱网站数据库设计 (增强版 v2.0)
-- 数据库：Oracle 11g
-- 创建日期：2025年12月
-- 设计规范：BCNF （Boyce-Codd Normal Form）
-- 
-- 本版本在原有20个表的基础上新增6个表：
-- MEAL_PLANS, MEAL_PLAN_ENTRIES, INGREDIENT_SUBSTITUTIONS,
-- RATING_HELPFULNESS, COMMENT_HELPFULNESS, USER_ALLERGIES
-- =====================================================================
...
-- 11. 食材过敏原表（INGREDIENT_ALLERGENS）
...
-- 14. 评价有用性记录表（RATING_HELPFULNESS） - 新增
...
-- 16. 评论有用性记录表（COMMENT_HELPFULNESS） - 新增
...
CREATE TABLE FOLLOWERS (
    ...
    CONSTRAINT fk_follow_follower FOREIGN KEY (follower_user_id) REFERENCES USERS(user_id) ON DELETE CASCADE,
    CONSTRAINT uk_follower UNIQUE (user_id, follower_user_id),
    CONSTRAINT ck_not_self_follow CHECK (user_id != follower_user_id)
);

-- 19. 用户活动日志表（USER_ACTIVITY_LOG）
...
-- 21. 清单内食谱表（COLLECTION_RECIPES）
CREATE TABLE COLLECTION_RECIPES (
    collection_recipe_id NUMBER(10) PRIMARY KEY,
    collection_id   NUMBER(10) NOT NULL,
    recipe_id       NUMBER(10) NOT NULL,
    added_at        TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_colr_collection FOREIGN KEY (collection_id) REFERENCES RECIPE_COLLECTIONS(collection_id) ON DELETE CASCADE,
    CONSTRAINT fk_colr_recipe FOREIGN KEY (recipe_id) REFERENCES RECIPES(recipe_id) ON DELETE CASCADE,
    CONSTRAINT uk_collection_recipe UNIQUE (collection_id, recipe_id)
);

-- 22. 购物清单表（SHOPPING_LISTS）
...
-- 23. 购物清单项目表（SHOPPING_LIST_ITEMS）
...
-- =====================================================================
-- 第五部分：膳食计划表（新增功能 26-27）
-- =====================================================================

-- 24. 膳食计划表（MEAL_PLANS） - 新增
CREATE TABLE MEAL_PLANS (
    ...
    start_date      DATE,
    end_date        DATE,
    is_public       VARCHAR2(1) DEFAULT 'N' CHECK (is_public IN ('Y', 'N')),
    created_at      TIMESTAMP DEFAULT SYSTIMESTAMP,
    updated_at      TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_mp_user FOREIGN KEY (user_id) REFERENCES USERS(user_id) ON DELETE CASCADE
);

-- 25. 膳食计划条目表（MEAL_PLAN_ENTRIES） - 新增
CREATE TABLE MEAL_PLAN_ENTRIES (
    entry_id        NUMBER(10) PRIMARY KEY,
    plan_id         NUMBER(10) NOT NULL,
    recipe_id       NUMBER(10) NOT NULL,
    meal_date       DATE NOT NULL,
    meal_type       VARCHAR2(50) NOT NULL CHECK (meal_type IN ('breakfast', 'lunch', 'dinner', 'snack')),
    notes           VARCHAR2(500),
    ...
    CONSTRAINT uk_plan_date_meal UNIQUE (plan_id, meal_date, meal_type)
);
...
-- 26. 食材替代品表（INGREDIENT_SUBSTITUTIONS） - 新增
CREATE TABLE INGREDIENT_SUBSTITUTIONS (
    ...
    approval_status       VARCHAR2(20) DEFAULT 'approved' CHECK (approval_status IN ('approved', 'pending', 'rejected')),
    created_at            TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_sub_original_ing FOREIGN KEY (original_ingredient_id) REFERENCES INGREDIENTS(ingredient_id),
    CONSTRAINT fk_sub_substitute_ing FOREIGN KEY (substitute_ingredient_id) REFERENCES INGREDIENTS(ingredient_id),
    CONSTRAINT fk_sub_user FOREIGN KEY (created_by_user_id) REFERENCES USERS(user_id),
    CONSTRAINT uk_substitution_pair UNIQUE (original_ingredient_id, substitute_ingredient_id)
);
...
CREATE INDEX idx_sub_substitute ON INGREDIENT_SUBSTITUTIONS(substitute_ingredient_id);
