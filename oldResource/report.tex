...
\hline
\textbf{膳食计划（MealPlans）} & 用户的一�?一月食谱安排?& 计划名称、时间范围、公开状态?\\
\hline
...
用户注册 -> 登录 -> 创建食谱 -> 添加基本信息 -> 添加食材 -> 设置烹饪步骤 -> 上传图片 -> 验证和预�?-> 发布
\end{lstlisting}

\textbf{1.1.3.2 食谱浏览与评价流程}
\begin{lstlisting}[language={}]
用户浏览 -> 搜索（按菜系/食材/难度等） -> 查看食谱详情 -> 查看评价和评�?-> 添加到收�?清单 -> 评价或评�?
\end{lstlisting}

\textbf{1.1.3.3 膳食规划流程}
\begin{lstlisting}[language={}]
查看推荐食谱 -> 创建膳食计划 -> 为每天安排食�?-> 系统整合购物清单 -> 生成购物清单 -> 管理购物进度
\end{lstlisting}

\textbf{1.1.3.4 社区互动流程}
\begin{lstlisting}[language={}]
浏览用户资料 -> 关注用户 -> 查看关注用户的食�?-> 与用户互动（评价、评论、私信等�?
\end{lstlisting}
...
本设计严格遵�?\textbf{BCNF（Boyce-Codd Normal Form）} 规范�?

\begin{itemize}
    \item \textbf{第一范式�?NF）}：所有字段包含原子值，不可再分�?
    \item \textbf{第二范式�?NF）}：所有非主键字段完全依赖于主键（特别是针对联合主键的表，�?\texttt{RECIPE\_INGREDIENTS}，\texttt{quantity} 依赖�?\texttt{recipe\_id} �?\texttt{ingredient\_id} 的组合）�?
    \item \textbf{第三范式�?NF）}：消除了所有传递依赖。例如，�?\texttt{RECIPES} 表中，用户信息只存储 \texttt{user\_id}，而不存储 \texttt{username}（通过连接查询获取），避免了数据冗余�?
...
\hline
ALLERGENS & allergen\_id & BCNF & 过敏原字�?\\
\hline
TAGS & tag\_id & BCNF & 标签字典 \\
\hline
USER\_ALLERGIES & user\_allergy\_id & BCNF & 用户过敏原关�?\\
\hline
RECIPES & recipe\_id & BCNF & 食谱主表，包含聚合字�?\\
\hline
RECIPE\_INGREDIENTS & recipe\_ingredient\_id & BCNF & N:M关系表，完全依赖于复合主�?\\
\hline
COOKING\_STEPS & step\_id & BCNF & 步骤序列保证唯一�?\\
\hline
NUTRITION\_INFO & nutrition\_id & BCNF & 一对一关系，完整分离营养数�?\\
\hline
INGREDIENT\_ALLERGENS & ingredient\_allergen\_id & BCNF & N:M关系�?\\
\hline
RECIPE\_TAGS & recipe\_tag\_id & BCNF & N:M关系�?\\
\hline
RATINGS & rating\_id & BCNF & 复合唯一键保证一人一�?\\
\hline
RATING\_HELPFULNESS & helpful\_id & BCNF & 跟踪每个"有用"投票，防重复 \\
\hline
COMMENTS & comment\_id & BCNF & 自引用支持嵌套评�?\\
\hline
...
\hline
USER\_ACTIVITY\_LOG & activity\_id & BCNF & 活动日志，支持审�?\\
\hline
RECIPE\_COLLECTIONS & collection\_id & BCNF & 用户创建的清�?\\
\hline
...
\subsubsection{1.5.1 查询操作（SELECT）}
...
\hline
\endfoot
\multirow{4}{*}{基础查询} & 按菜系搜索食�?& 根据菜系、发布状态筛选食谱，按评分和浏览量排�?\\
\cline{2-3}
& 按难度和时间查询食谱 & 筛选特定难度和时间限制内的食谱 \\
\cline{2-3}
& 查询用户的所有食�?& 获取指定用户发布的所有未删除食谱 \\
\cline{2-3}
& 查询用户收藏的食�?& 获取指定用户收藏的食谱列�?\\
\hline
...
\hline
\multirow{3}{*}{评价与评论} & 查询食谱评价 & 获取食谱的评分、评论内容及有用性计�?\\
\cline{2-3}
& 查询食谱评论 & 递归查询食谱的评论树（含回复�?\\
\cline{2-3}
& 查询用户评价 & 获取指定用户发出的所有评价记�?\\
\hline
\multirow{4}{*}{社交与推荐} & 查询关注/粉丝 & 获取用户的关注列表和粉丝列表 \\
\cline{2-3}
& 关注者食谱推�?& 推荐用户关注的创作者发布的新食�?\\
\cline{2-3}
& 特定食材搜索 & 查找包含指定食材（如鸡蛋、面粉）的食�?\\
\cline{2-3}
& 安全食谱推荐 & 排除用户过敏食材的食谱推�?\\
\hline
\multirow{4}{*}{高级分析} & 热门食谱排行 & 基于评分、评论数、浏览数计算热度并排�?\\
\cline{2-3}
...
\subsubsection{1.5.2 数据变更操作}

\begin{longtable}{|L{2.5cm}|L{2.5cm}|L{3cm}|L{7cm}|}
\caption{数据变更操作归纳} \\
\hline
...
\hline
\endfoot
\multirow{9}{*}{插入 (INSERT)} & \multirow{2}{*}{用户与食谱} & 新用户注�?& 创建新的用户账户信息 \\
\cline{3-4}
& & 发布新食�?& 事务处理：插入食谱、食材、步骤、营养信息并记录日志 \\
\cline{2-4}
& \multirow{4}{*}{交互} & 用户评价/评论 & 添加评分、评论内容，并更新统计数�?\\
\cline{3-4}
& & 收藏食谱 & 将食谱添加到用户收藏�?\\
\cline{3-4}
& & 关注用户 & 建立用户间的关注关系 \\
...
& & 添加计划�?& 向膳食计划中添加食谱 \\
\cline{3-4}
& & 创建购物清单 & 生成清单并自动导入计划所需的食�?\\
\cline{2-4}
& 设置 & 添加过敏�?& 记录用户的过敏食材信�?\\
\hline
\multirow{4}{*}{更新 (UPDATE)} & \multirow{2}{*}{编辑} & 更新食谱信息 & 修改食谱详情及更新时�?\\
\cline{3-4}
...
& & 购物清单勾�?& 标记清单中的物品为已购买 \\
\hline
\multirow{5}{*}{删除 (DELETE)} & \multirow{5}{*}{清理} & 删除评价/收藏 & 物理删除评价记录或取消收�?\\
\cline{3-4}
& & 清空购物清单 & 删除指定清单内的所有条�?\\
\cline{3-4}
& & 取消关注 & 移除关注关系 \\
...
\hline
\textbf{UK} & \texttt{RECIPE\_ID} & 确保一对一关系，一个食谱只有一条营养记�?\\
\hline
...
\textbf{FK} & \texttt{RECIPE\_ID} & 引用 \texttt{RECIPES(RECIPE\_ID)} \\
\hline
\textbf{UK} & \texttt{(USER\_ID, RECIPE\_ID)} & 确保不重复收�?\\
\hline
...
\hline
\textbf{FK} & \texttt{USER\_ID} & 引用 \texttt{USERS(USER\_ID)} (被关注�? \\
\hline
\textbf{FK} & \texttt{FOLLOWER\_USER\_ID} & 引用 \texttt{USERS(USER\_ID)} (关注�? \\
\hline
\textbf{CK} & \texttt{USER\_ID != FOLLOWER\_USER\_ID} & 防止自关�?\\
\hline
...
\textbf{20. USER\_ACTIVITY\_LOG (用户活动日志�?}
\begin{longtable}{|L{2cm}|L{8cm}|L{5cm}|}
\hline
\textbf{约束类型} & \textbf{字段/表达式} & \textbf{详情/规则} \\
\hline
...
\textbf{FK} & \texttt{USER\_ID} & 引用 \texttt{USERS(USER\_ID)} \\
\hline
\textbf{FK} & \texttt{RECIPE\_ID} & 引用 \texttt{RECIPES(RECIPE\_ID)} (可�? \\
\hline
...
\textbf{21. RECIPE\_COLLECTIONS (食谱收藏清单�?}
\begin{longtable}{|L{2cm}|L{8cm}|L{5cm}|}
\hline
\textbf{约束类型} & \textbf{字段/表达式} & \textbf{详情/规则} \\
\hline
...
\textbf{24. SHOPPING\_LIST\_ITEMS (购物清单项目�?}
\begin{longtable}{|L{2cm}|L{8cm}|L{5cm}|}
\hline
\textbf{约束类型} & \textbf{字段/表达式} & \textbf{详情/规则} \\
\hline
...
\hline
\textbf{CK} & \texttt{START\_DATE <= END\_DATE} & 结束日期必须晚于或等于开始日�?\\
\hline
\textbf{CK} & \texttt{IS\_PUBLIC} & 值域: \texttt{('Y', 'N')} \\
\hline
\textbf{DF} & \texttt{IS\_PUBLIC} & \texttt{'Y'} \\
...
\textbf{26. MEAL\_PLAN\_ENTRIES (膳食计划条目�?}
\begin{longtable}{|L{2cm}|L{8cm}|L{5cm}|}
\hline
\textbf{约束类型} & \textbf{字段/表达式} & \textbf{详情/规则} \\
\hline
\textbf{PK} & \texttt{(PLAN\_ID, RECIPE\_ID, MEAL\_DATE)} & \textbf{三字段联合主键}，确保同一计划同一天同一食谱不重�?\\
\hline
...
\subsection{1.7 表的详细设计（物理模型）}

\subsubsection{1.7.1 表的分类和数量统计}
...
\hline
\textbf{核心基础} & USERS, INGREDIENTS, UNITS, ALLERGENS, TAGS, USER\_ALLERGIES & 6 & 系统基础数据�?\\
\hline
...
\subsubsection{1.7.2 核心表详细设计}

\textbf{用户核心表}

\textbf{1. USERS表（用户表）}
\begin{longtable}{|L{3cm}|L{2.5cm}|c|c|L{2cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
USER\_ID & NUMBER(10) & �?& �?& PK & 用户唯一标识，自�?\\
\hline
USERNAME & VARCHAR2(50) & �?& & UK & 用户名，唯一 \\
\hline
EMAIL & VARCHAR2(100) & �?& & UK & 邮箱，唯一，用于登录和验证 \\
\hline
PASSWORD\_HASH & VARCHAR2(255) & �?& & & 加密后的密码哈希�?\\
\hline
...
\hline
UPDATED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 最后修改时�?\\
\hline
\endlongtable}
...
\hline
DESCRIPTION & VARCHAR2(100) & & & & 单位描述和转换信�?\\
\hline
CREATED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 创建时间 \\
\hline
\endlongtable}
...
\begin{longtable}{|L{3.5cm}|L{2.5cm}|c|c|L{1.8cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
...
\hline
IS\_PUBLISHED & VARCHAR2(1) & �?& & CK & Y/N，是否发�?\\
\hline
...
CREATED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 创建时间 \\
\hline
UPDATED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 最后更新时�?\\
\hline
\endlongtable}
...
\hline
INGREDIENT\_ID & NUMBER(10) & �?& �?PK2) & FK & 食材ID，联合主键第二部�?\\
\hline
...
\hline
RECIPE\_ID & NUMBER(10) & �?& �?& PK, FK, UK & 食谱ID（一对一关系�?\\
\hline
...
\hline
ALLERGEN\_ID & NUMBER(10) & �?& �?PK2) & FK & 过敏原ID，联合主键第二部�?\\
\hline
\endlongtable}

\textbf{11. INGREDIENT\_SUBSTITUTIONS 表（食材替代品关联表 - 多对多）}
\begin{longtable}{|L{3.5cm}|L{2.5cm}|c|c|L{2cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
ORIGINAL\_INGREDIENT\_ID & NUMBER(10) & �?& �?PK1) & FK & 原始食材ID，联合主键第一部分 \\
\hline
SUBSTITUTE\_INGREDIENT\_ID & NUMBER(10) & �?& �?PK2) & FK & 替代食材ID，联合主键第二部�?\\
\hline
...
\hline
USER\_ID & NUMBER(10) & �?& �?PK2) & FK & 投票者用户ID，联合主键第二部�?\\
\hline
HELPFUL\_VOTES & NUMBER(10) & & & DF=0 & 有用投票�?\\
\hline
...
\textbf{17. FOLLOWERS 表（用户关注关系�?- 多对多自引用）}
\begin{longtable}{|L{3.5cm}|L{2.5cm}|c|c|L{2cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
...
\hline
FOLLOWER\_USER\_ID & NUMBER(10) & �?& �?PK2) & FK & 关注者用户ID，联合主键第二部�?\\
\hline
...
RECIPE\_ID & NUMBER(10) & �?& �?PK1) & FK & 食谱ID，联合主键第一部分 \\
\hline
TAG\_ID & NUMBER(10) & �?& �?PK2) & FK & 标签ID，联合主键第二部�?\\
\hline
ADDED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 添加时间 \\
\hline
\endlongtable}
...
\hline
RECIPE\_ID & NUMBER(10) & �?& �?PK2) & FK & 食谱ID，联合主键第二部�?\\
\hline
ADDED\_AT & TIMESTAMP & �?& & DF=SYSDATE & 添加时间 \\
\hline
\endlongtable}
...
\textbf{24. SHOPPING\_LIST\_ITEMS 表（购物清单项目�?- 多对多）}
\begin{longtable}{|L{3cm}|L{2.5cm}|c|c|L{2cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
...
USER\_ID & NUMBER(10) & �?& & FK & 用户ID \\
\hline
PLAN\_NAME & VARCHAR2(100) & �?& & & 计划名称 \\
\hline
DESCRIPTION & VARCHAR2(500) & & & & 计划描述 \\
\hline
START\_DATE & DATE & �?& & CK & 开始日�?\\
\hline
...
\textbf{26. MEAL\_PLAN\_ENTRIES 表（膳食计划条目�?- 多对多）}
\begin{longtable}{|L{3cm}|L{2.5cm}|c|c|L{2cm}|L{4cm}|}
\hline
\textbf{字段名} & \textbf{数据类型} & \textbf{非空} & \textbf{主键} & \textbf{约束} & \textbf{说明} \\
\hline
...
RECIPE\_ID & NUMBER(10) & �?& �?PK2) & FK & 食谱ID，联合主键第二部�?\\
\hline
MEAL\_DATE & DATE & �?& �?PK3) & & 该食谱的日期，联合主键第三部�?\\
\hline
...
\cline{2-4}
& \textbf{USER\_CONTRIBUTION\_SCORE} (贡献评分) & \textbf{排行榜计算}。基于食谱、粉丝、互动量加权计算用户的社区贡献分�?& Users, Ratings, Comments \\
\hline
\multirow{4}{*}{食谱相关} & \textbf{RECIPE\_DETAIL} (食谱详情) & \textbf{详情页展示}。封装食谱基本信息、作者信息及营养成分，屏蔽已删除数据�?& Recipes, Users, Nutrition \\
\cline{2-4}
& \textbf{POPULAR\_RECIPES} (热门食谱) & \textbf{推荐算法}。基于评�?50\%)、评价数(30\%)、浏览量(20\%)计算热度分�?& Recipes, Ratings \\
\cline{2-4}
...
\cline{2-4}
& \textbf{TOP\_CONTRIBUTORS} (顶级贡献�? & \textbf{社区发现}。按综合指标排名的优质创作者列表�?& Users, Ratings \\
\hline
...
\subsubsection{数据加密策略}

用户密码使用哈希存储�?

\section{实现部分}

\subsection{2.1 表格创建（可同时包括约束条件的创建）}

为了节省正文篇幅，sql代码实现部分见附录，正文部分仅展示运行结果�?

\subsubsection{2.2.1 创建序列（主键自增）}

\subsection{2.2 数据录入}

\subsection{2.3 视图创建和使用}

\subsection{2.4 常见操作的实例演示}

\subsection{2.5 数据库安全的实现}

\subsubsection{2.5.1 数据库用户角色划分}
...
\subsection{3.2 难点与不足分析}

\subsection{3.3 收获或体会及建议}
