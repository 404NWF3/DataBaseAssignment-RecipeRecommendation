...
\hline
\textbf{膳食计划（MealPlans）} & 用户的一周/一月食谱安排& 计划名称、时间范围、公开状�✓\\
\hline
...
\begin{lstlisting}[language={}]
用户浏览 -> 搜索（按菜系/食材/难度等） -> 查看食谱详情 -> 查看评价和评论 -> 添加到收藏清单 -> 评价或评价
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{第二范式（1NF）}：所有非主键字段完全依赖于主键（特别是针对联合主键的表，如\texttt{RECIPE\_INGREDIENTS}，\texttt{quantity} 依赖于\texttt{recipe\_id} ✓\texttt{ingredient\_id} 的组合）。
    \item \textbf{第三范式（1NF）}：消除了所有传递依赖。例如，✓\texttt{RECIPES} 表中，用户信息只存储 \texttt{user\_id}，而不存储 \texttt{username}（通过连接查询获取），避免了数据冗余�✓
