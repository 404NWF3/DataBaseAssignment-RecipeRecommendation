...
AllRecipes是全球最大的食谱分享平台之一，拥有数百万用户和数百万条食谱。该网站以其简洁直观的界面、丰富的食谱内容和强大的社区功能而闻名。以下为五点核心功能：
...
\begin{itemize}
    ...
    \item \textbf{用户交互与社交功能}：包括食谱评价、评论、收藏、关注其他用户等丰富的互动方式，建立社区生态，提高用户体验。
    \item \textbf{个性化食谱管理}：用户可以创建多个食谱收藏清单、购物清单、膳食计划等，使每日的烹饪更加便捷，饮食更有规划。
...
\hline
\endfoot
\multirow{2}{*}{基础查询} & 按菜系搜索食谱& 根据菜系/发布状态筛选食谱，按评分/浏览量排序\\
\cline{2-3}
& 查询用户的所有食谱& 获取指定用户发布的所有未删除食谱 \\
\cline{2-3}
& 查询用户收藏的食谱& 获取指定用户收藏的食谱列✓\\
\hline
\multirow{3}{*}{复杂关联查询} & 查询食谱详情 & 获取食谱的基本信息：作者、营养成分及标签等\\
\cline{2-3}
& 查询食谱食材 & 列出食谱所需的所有食材、用量及单位 \\
\cline{2-3}
& 查询烹饪步骤 & 按顺序获取食谱的制作步骤和图片\\
\hline
\multirow{3}{*}{评价与评论} & 查询食谱评价 & 获取食谱的评分、评论内容及有用性\\
\cline{2-3}
...
/suspended \\
\hline
TOTAL\_FOLLOWERS & NUMBER(10) & √& DF=0 & 粉丝数量
（冗余字段）\\
\hline
TOTAL\_RECIPES & NUMBER(10) & √& DF=0 & 发布的食谱数量
（冗余字段）\\
\hline
...
\cline{2-4}
& \textbf{USER\_FEED} (用户动态& \textbf{首页Feed流}。聚合用户关注对象的最新动态（发布、评分、评论）✓& Followers, Activity\_Log \\
\cline{2-4}
& \textbf{TOP\_CONTRIBUTORS} (高级贡献者) & \textbf{社区发现}。按综合指标排名的优质创作者列表�✓& Users, Ratings \\
\hline
